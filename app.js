/************ KONFIG ************/
const WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbxR-LcgCukny9hSmX9611U-tv1gb1vnaHYP7Sb7LWeY_6HgqnIxYD6UuDua6vnSv2s/exec'; // <- hardcoded
const PDF_FILE = 'of019.pdf'; // pastikan fail ini ada di repo

/************ UTIL DOM ************/
const $ = s => document.querySelector(s);

/************ PETA KOORDINAT ************/
// Isi koordinat sedikit demi sedikit guna mode Calibrate
const MAP = {
  // Contoh asas (anda akan ganti ikut hasil calibrate):
  // name:{x:105,y:145}, mrn:{x:535,y:145}, nric_passport:{x:535,y:170},
  specimen_type: {
    // BLOOD:{x:128,y:495}, URINE:{x:208,y:495}, FAECES:{x:280,y:495}, SWAB:{x:350,y:495}, OTHERS:{x:430,y:495}
  },
  fasting_yes: null, // {x:530,y:540}
  fasting_no:  null, // {x:575,y:540}
  profile: {
    // 'Anaemia':{x:38,y:635}, 'Diabetes':{x:38,y:695}, ...
  },
  individual: {
    // 'FBC':{x:305,y:705}, 'HbA1C':{x:305,y:750}, ...
  },
  other_tests_rows: [
...
async function ensureExcelLoaded(){
  if (cacheExcel.length) return true;
  const f = $('#excel').files && $('#excel').files[0];
  if (!f) return false;
  cacheExcel = await readExcel(f);
  $('#list').textContent = JSON.stringify(cacheExcel.slice(0,10), null, 2);
  return cacheExcel.length > 0;
}
...
$('#excel').addEventListener('change', async ()=>{
  cacheExcel = [];
  await ensureExcelLoaded(); // auto-parse & preview bila tukar fail
});

$('#btnUpload').addEventListener('click', async ()=>{
  if (!(await ensureExcelLoaded())) return alert('Sila pilih Excel dahulu');
  const res = await fetch(WEB_APP_URL, {
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify(cacheExcel) // INSERT default
  });
